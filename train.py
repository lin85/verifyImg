...
samples = glob.glob(r'C:\Users\Administrator\Desktop\img\*.jpg')  # 获取所有样本图片
np.random.shuffle(samples)  # 将图片打乱

nb_train = 4000  # 共有5万样本，4.5万用于训练，5k用于验证
train_samples = samples[:nb_train]
test_samples = samples[nb_train:]

test = glob.glob(r'C:\Users\Administrator\Desktop\verifycode\test\*.jpg')
test_samples += test
...
predicts = [Dense(36, activation='softmax')(Dropout(0.5)(base_model.output)) for i in range(6)]
...
def data_generator(data, batch_size):  # 样本生成器，节省内存
    while True:
        ...
        x, y = [], []
        for img in batch:
            real_num = img.split("\\")[-1].split("_")[0].split(".")[0]
            if len(real_num) == 6:
